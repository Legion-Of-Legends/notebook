...
with open("../Tasks/Task Setter2.md", "w") as f:
    ...
    for i in task_setter_data.keys():
        ...
        for j in task_setter_data[i].keys():
            ...
            new_setter_write+=f"|{j}{" "*(max_task_title_len-len(j))}|{task_setter_data[i][j]['from']}{' '*(max_date_len-len(task_setter_data[i][j]['from']))}|{task_setter_data[i][j]['to']}{' '*(max_date_len-len(task_setter_data[i][j]['to']))}|{task_setter_data[i][j]['off_days']}{' '*7}|{task_setter_data[i][j]['on_days']}{' '*6}|{current_weekoff}{' '*(max_weekoff_len-len(current_weekoff))}|status  |{task_setter_data[i][j]['description']}{' '*(max_description_len-len(task_setter_data[i][j]['description']))}|\n"
